...
.DESCRIPTION
    ...
    SCORING ENGINE: https://scoring.mlp.local:443
...
OPTIONS:
    ...
    -Phase4        SSH Removal (removes/blocks SSH - not required on Windows)
...
# NOTE: Setting Enabled=0 alone causes a popup dialog ("Windows Script Host access is
# disabled on this machine") every ~10 seconds whenever anything tries to invoke WSH.
# SilentTerminate=1 suppresses that dialog. WSH is still fully disabled - scripts still
# cannot run - they just fail silently instead of nagging. Both HKLM (system-wide) and
# HKCU (current user session) are set to cover all cases.
try {
