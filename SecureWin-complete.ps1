...
if ($EnableSSHHardening) {
    ...
    # Get-WindowsCapability can throw a COMException when account SIDs are in flux (e.g. after user deletions).
    # Wrap in try-catch and treat any exception as "not installed" to avoid crashing Phase 4.
    ...
    try {
        ...
        Write-BlueTeamLog "Get-WindowsCapability failed (common after user account changes): $_" "WARNING"
        Write-BlueTeamLog "Assuming OpenSSH Server is not installed - skipping SSH hardening" "WARNING"
    }
...
# NOTE on SilentTerminate: Setting Enabled=0 alone causes Windows to show a popup dialog
# ("Windows Script Host access is disabled on this machine") every ~10 seconds whenever
# anything (login scripts, scheduled tasks, Windows internals) tries to call WSH.
# SilentTerminate=1 suppresses that dialog completely. WSH is still fully disabled -
# scripts still cannot run - they just fail silently instead of nagging the user.
# We also set both HKLM (system-wide) and HKCU (current user session) to cover all cases.
try {
    Write-BlueTeamLog "Disabling Windows Script Host (silent - no popup)..." "INFO"

    $wshHKLM = "HKLM:\SOFTWARE\Microsoft\Windows Script Host\Settings"
    if (-not (Test-Path $wshHKLM)) { New-Item -Path $wshHKLM -Force | Out-Null }
    Set-ItemProperty -Path $wshHKLM -Name "Enabled"         -Value 0 -Type DWord -Force
    Set-ItemProperty -Path $wshHKLM -Name "SilentTerminate" -Value 1 -Type DWord -Force

    $wshHKCU = "HKCU:\SOFTWARE\Microsoft\Windows Script Host\Settings"
    if (-not (Test-Path $wshHKCU)) { New-Item -Path $wshHKCU -Force | Out-Null }
    Set-ItemProperty -Path $wshHKCU -Name "Enabled"         -Value 0 -Type DWord -Force
    Set-ItemProperty -Path $wshHKCU -Name "SilentTerminate" -Value 1 -Type DWord -Force

    Add-Change "System Security" "Windows Script Host" "Disabled + SilentTerminate" "WSH disabled, popup dialog suppressed"
    Write-BlueTeamLog "Windows Script Host disabled silently (no more popups)" "SUCCESS"
} catch {
