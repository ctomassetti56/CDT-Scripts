...
OPTIONS:
    ...
    -Phases        Run specific phases by number (e.g., -Phases 1,3,8)
...
PHASE DESCRIPTIONS:
    Phase 1: User Account Management
        ...
        - Audit group memberships (16 privileged groups)
        - Detect blank passwords and suspicious SIDs
        - Reset ALL passwords to team password
        - Generate audit report

    Phase 2: Password Policy
        - Min 16 char password length
        - 90 day max age, 1 day min age
        - 3 attempt lockout threshold

    Phase 3: Firewall
        - Enable firewall on all profiles
        - Configure allowed ports for scored services
        - Whitelist scoring/jumpbox IPs

    Phase 4: SSH
        - Restrict to authorized users only
        - Strong ciphers and authentication

    Phase 5: Network Security
        - Disable SMBv1, LLMNR, NetBIOS
        - Maintain SMBv2/3 for scoring

    Phase 6: Backdoor Detection
        - Scan scheduled tasks, services, startup locations

    Phase 7: System Hardening
        - UAC, DEP, Windows Defender
        - Disable AutoRun, WSH, PowerShell v2
...
EXAMPLES:
    .\SecureWin.ps1                          # Run all phases
    .\SecureWin.ps1 -All                     # Run all phases (explicit)
    .\SecureWin.ps1 -Phase1                  # Run only Phase 1
    .\SecureWin.ps1 -Phase1 -Phase3 -Phase8  # Run multiple phases (switches)
    .\SecureWin.ps1 -Phases 1,3,8              # Run multiple phases (numbers)
    .\SecureWin.ps1 -Help                    # Show this menu
...
# Parse -Phases (supports: -Phases 1,3,8  OR  -Phases "1,3,8"  OR  -Phases 1 3 8)
$RequestedPhaseNumbers = @()

if ($Phases) {
    foreach ($p in $Phases) {
        foreach ($token in ($p -split ',')) {
            $t = $token.Trim()
            if ([string]::IsNullOrWhiteSpace($t)) { continue }

            if ($t -notmatch '^\d+$') {
                Write-Error "Invalid phase value '$t'. Use numbers 1-8 (example: -Phases 1,3,8)."
                exit 1
            }

            $n = [int]$t
            if ($n -lt 1 -or $n -gt 8) {
                Write-Error "Invalid phase number '$n'. Valid range is 1-8."
                exit 1
            }

            $RequestedPhaseNumbers += $n
        }
    }
}

# Include explicit switch phases too (allow mixing switches + -Phases)
if ($Phase1) { $RequestedPhaseNumbers += 1 }
if ($Phase2) { $RequestedPhaseNumbers += 2 }
if ($Phase3) { $RequestedPhaseNumbers += 3 }
if ($Phase4) { $RequestedPhaseNumbers += 4 }
if ($Phase5) { $RequestedPhaseNumbers += 5 }
if ($Phase6) { $RequestedPhaseNumbers += 6 }
if ($Phase7) { $RequestedPhaseNumbers += 7 }
if ($Phase8) { $RequestedPhaseNumbers += 8 }

# Normalize: unique + sorted
$RequestedPhaseNumbers = $RequestedPhaseNumbers | Sort-Object -Unique

$AnyPhaseSelection = ($RequestedPhaseNumbers.Count -gt 0)
$NoArgs            = (-not $PSBoundParameters.Count)
$FullRun           = ($All -or $NoArgs)

$RunPhase1 = $FullRun -or ($RequestedPhaseNumbers -contains 1)
$RunPhase2 = $FullRun -or ($RequestedPhaseNumbers -contains 2)
$RunPhase3 = $FullRun -or ($RequestedPhaseNumbers -contains 3)
$RunPhase4 = $FullRun -or ($RequestedPhaseNumbers -contains 4)
$RunPhase5 = $FullRun -or ($RequestedPhaseNumbers -contains 5)
$RunPhase6 = $FullRun -or ($RequestedPhaseNumbers -contains 6)
$RunPhase7 = $FullRun -or ($RequestedPhaseNumbers -contains 7)
$RunPhase8 = $FullRun -or ($RequestedPhaseNumbers -contains 8)

# "Individual" means: user explicitly selected one or more phases (and did NOT request -All)
$RunningIndividualPhase = (-not $FullRun) -and $AnyPhaseSelection

# Build selected phase names (for banners/log naming)
$SelectedPhaseNames = foreach ($n in $RequestedPhaseNumbers) {
    switch ($n) {
        1 { "Phase 1: User Account Management" }
        2 { "Phase 2: Password Policy Hardening" }
        3 { "Phase 3: Firewall Hardening" }
        4 { "Phase 4: SSH Hardening" }
        5 { "Phase 5: Network Security" }
        6 { "Phase 6: Backdoor Detection" }
        7 { "Phase 7: System Hardening" }
        8 { "Phase 8: Audit Logging" }
    }
}

if ($RunningIndividualPhase) {
    $phaseList = if ($SelectedPhaseNames.Count) { ($SelectedPhaseNames -join ", ") } else { "Selected phases" }

    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "RUNNING SELECTED PHASE(S): $phaseList" -ForegroundColor Yellow
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
}

# Set log file name (include phase marker when running selected phases)
if ($RunningIndividualPhase) {
    $phaseLabel = if ($RequestedPhaseNumbers.Count) { ($RequestedPhaseNumbers -join '-') } else { "Custom" }
    $LogFilePath = "C:\BlueTeam\Logs\Hardening-Phases-$phaseLabel-$(Get-Date -Format 'yyyy-MM-dd-HHmmss').log"
} else {
...
if ($shouldRestart) {
    ...
    # Individual phase summary (no restart / no full report)
    Write-Host ""
    Write-Host "========================================" -ForegroundColor Green
    Write-Host "Individual Phase Complete" -ForegroundColor Green
    Write-Host "Phase executed successfully." -ForegroundColor Green
